<template>
    <div class="min-h-full">
        <div class="bg-gray-800" :class="{'pb-32': $slots['title'] || $slots['header']}">
            <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
                        ...
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <Link href="/dashboard" as="button">
                                    <img class="h-8 w-8" src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=500" alt="Sheet Generator" />
                                </Link>
                            </div>
                            <div class="hidden md:block">
                                <div class="ml-10 flex items-baseline space-x-4">
                                    <Link
                                        ...
                                        :key="item.name"
                                        :class="[$page.url.startsWith(item.href) ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'px-3 py-2 rounded-md text-sm font-medium']"
                                        :href="item.href"
                ...
                <DisclosurePanel class="md:hidden">
                    <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
                        <DisclosureButton v-for="item in navigation" :key="item.name" as="a" :href="item.href" :class="[$page.url === item.href ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block px-3 py-2 rounded-md text-base font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</DisclosureButton>
                    </div>
            ...
            <header class="py-10" v-if="$slots['title']">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <h3 class="text-3xl font-bold tracking-tight text-white"><slot name="title"></slot></h3>
                </div>
            </header>
        </div>
